...
So far all our examples of the force layout have been just bubbles. Circles. But, remember, D3 layouts don't restrict to a particular visual representation. So, we can use squares, images (faces), or even states. D3 force layout don't care. 
...
First up, my circle-based layout examples. Talk about using sine/cosine for radial placement. Then look at the 'expanding center node' example - which kind of looks like a Kill Bill fight. 

Finally, we end with two experiments created by Moritz Stefaner. Both deal with doubling up on force layout components. The first uses two positions for nodes. One for the visual and one to push the visual to the right location. The second uses two separate force layouts. One for nodes one for labels. 
